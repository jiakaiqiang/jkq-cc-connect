...
function summarizeRecentMessages(messages, currentInput) {
    const lines = [];
    for (const message of messages.slice(-6)) {
        const content = compactText(message.content);
        if (!content)
            continue;
        if (message.type === 'user' && content === compactText(currentInput))
            continue;
...
export function buildExecutionInput(options) {
    const { text, targetTool, toolHasNativeSession, lastSuccessfulTool, handoffFromTool, conversationState, recentMessages, } = options;
    const continuingSameTool = targetTool === lastSuccessfulTool && toolHasNativeSession && !handoffFromTool;
    if (continuingSameTool)
        return text;
    const sections = [];
