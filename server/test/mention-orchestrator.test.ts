...
test('WSGateway mention input persists and broadcasts the multi-agent orchestration lifecycle', async () => {
  ...
  const persistedEntries = getMessages(session.id)
    ...
    .map((message) => ({
      ...
      sourceLabel: message.metadata.sourceLabel,
      sourceAgent: message.metadata.sourceAgent,
      sourceAgentLabel: message.metadata.sourceAgentLabel,
  ...
  const broadcastEntries = broadcasts
    ...
    .map((message) => ({
      ...
      sourceAgentLabel: message.sourceAgentLabel,
  ...
  assert.deepEqual(persistedEntries, [
    ...
    },
    {
      ...
      source: 'claude',
      sourceLabel: 'Claude / lead -> peer',
      sourceAgent: 'claude:lead',
      sourceAgentLabel: 'lead',
      senderType: 'agent',
    ...
    },
    {
      ...
      sourceAgent: 'claude:peer',
      sourceAgentLabel: 'peer',
      senderType: 'agent',
