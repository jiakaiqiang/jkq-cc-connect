...
let cache: { expiresAt: number; tools: VibeToolInfo[] } | null = null
...
function sanitizeWhitespace(text: string | null | undefined): string {
...
function extractFirstParagraph(markdown: string): string {
  ...
  for (const block of body.split(/\r?\n\r?\n/)) {
    const paragraph = sanitizeWhitespace(
      block
        .split(/\r?\n/)
...
function unique(items: Array<string | null | undefined>, limit = 8): string[] {
  const result: string[] = []
...
function inferAgentCapabilities(name: string, description: string): string[] {
...
function formatToolCapability(toolName: string): string {
  const normalized = toolName.toLowerCase()

  if (normalized === 'read') return '可读取文件与代码上下文'
  if (normalized === 'write') return '可写入或创建工作区文件'
  if (normalized === 'edit') return '可直接编辑现有文件'
  if (normalized === 'bash' || normalized === 'shell') return '可执行终端命令'
  if (normalized === 'grep' || normalized === 'glob' || normalized === 'search') return '可检索代码与文件'
  if (normalized === 'web') return '可访问网页与在线资料'
  if (normalized === 'mcp') return '可调用 MCP 服务'

  return `可调用 ${toolName} 工具`
}

function formatOpenCodePermission(permission: string): string {
  const normalized = permission.toLowerCase()

  if (normalized === '*') return '通用工具执行'
  if (normalized === 'read') return '读取工作区与本地文件'
  if (normalized === 'write') return '修改工作区文件'
  if (normalized === 'bash' || normalized === 'shell') return '执行终端命令'
  if (normalized === 'external_directory') return '访问外部目录'
  if (normalized === 'web') return '访问网络资源'
  if (normalized === 'mcp') return '调用 MCP 服务'

  return permission
}

function extractOpenCodePermissionRules(body: string): { allow: string[]; ask: string[] } {
  ...
  for (const match of body.matchAll(/"permission":\s*"([^"]+)"[\s\S]{0,160}?"action":\s*"([^"]+)"/g)) {
    ...
    if (action === 'allow') allow.add(permission)
    if (action === 'ask') ask.add(permission)
  }

  return { allow: [...allow], ask: [...ask] }
}

function buildMarkdownDocument(title: string, description: string, capabilities: string[], extraSections: Array<{ title: string; items: string[] }> = []) {
  const sections = [
    `# ${title}`,
    '',
    description,
...
function buildClaudeAgents(installed: boolean): ToolAgentInfo[] {
    ...
    buildAgent(
      ...
      buildMarkdownDocument(
        ...
        installed
          ...
          : '当前环境中未检测到 Claude CLI，因此该 Agent 暂不可用。',
        defaultCapabilities,
        [
  ...
  for (const fileName of readdirSync(agentsDir)) {
    ...
    try {
      ...
      agents.push(buildAgent(
        ...
        installed ? '可用' : '缺失',
        description,
        capabilities.length ? capabilities : ['按自定义提示词处理专项任务'],
        `${name}.md`,
        filePath,
        markdown,
      ))
...
function buildCodexAgents(installed: boolean, authenticated: boolean): ToolAgentInfo[] {
  const capabilities = unique([
    '多文件代码修改与补丁输出',
    '代码审查与回归风险识别',
    '终端命令、构建与测试执行',
  ...
  const state: VibeToolState = !installed ? 'missing' : authenticated ? 'ready' : 'limited'
...
function parseOpenCodeAgentSections(output: string): Array<{ name: string; rawStatus: string; body: string }> {
  const lines = output.split(/\r?\n/)
  const sections: Array<{ name: string; rawStatus: string; body: string }> = []
  let current: { name: string; rawStatus: string; body: string } | null = null
...
function mapOpenCodeStatus(rawStatus: string): { state: VibeToolState; statusText: string } {
  const normalized = rawStatus.toLowerCase()
  if (normalized.includes('primary')) return { state: 'ready', statusText: '主 Agent' }
  if (normalized.includes('subagent')) return { state: 'ready', statusText: '子 Agent' }
  if (normalized.includes('active') || normalized.includes('running')) return { state: 'ready', statusText: '运行中' }
  if (normalized.includes('default')) return { state: 'ready', statusText: '默认' }
  return { state: 'limited', statusText: rawStatus || '已检测' }
}

function summarizeOpenCodeCapabilities(body: string): string[] {
...
function buildOpenCodeAgentMarkdown(name: string, statusText: string, description: string, capabilities: string[], body: string) {
  const { allow, ask } = extractOpenCodePermissionRules(body)
...
function buildOpenCodeAgents(installed: boolean, configured: boolean): ToolAgentInfo[] {
  if (!installed) {
    return [
      buildAgent(
        'opencode:default',
        ...
        undefined,
        buildMarkdownDocument('OpenCode Default Agent', '当前环境中未检测到 OpenCode CLI。', ['等待安装 OpenCode CLI']),
      ),
    ]
  }
  ...
  if (!output) {
    ...
    return [
      buildAgent(
        ...
        'OpenCode Default Agent',
        undefined,
        buildMarkdownDocument('OpenCode Default Agent', description, capabilities),
      ),
    ]
  }
  ...
  if (!sections.length) {
    return [
      buildAgent(
        ...
        configured ? 'ready' : 'error',
        configured ? '可用' : '需配置',
        'OpenCode 已安装，但未解析到结构化 Agent 信息。',
        ['命令执行', '模型调用'],
        'OpenCode Default Agent',
        undefined,
        buildMarkdownDocument('OpenCode Default Agent', 'OpenCode 已安装，但未解析到结构化 Agent 信息。', ['命令执行', '模型调用']),
      ),
    ]
  }
  ...
  return sections.map((section) => {
    ...
    return buildAgent(
      `opencode:${section.name}`,
      section.name,
      finalState,
      finalStatus,
      description,
      capabilities.length ? capabilities : ['命令执行', '文件读写'],
      `${section.name}.md`,
      undefined,
...
function detectClaude(): VibeToolInfo {
...
function detectCodex(): VibeToolInfo {
...
function detectOpenCode(): VibeToolInfo {
...
export function getVibeTools(force = false): VibeToolInfo[] {
...
function findTool(tools: VibeToolInfo[], id: VibeToolId) {
  return tools.find(tool => tool.id === id)
}

function scorePromptForTool(text: string, toolId: VibeToolId) {
  const input = text.toLowerCase()
  let score = 0

  if (toolId === 'claude') {
    score += 40
    if (/(fix|implement|modify|build|debug|refactor|repair|feature|修复|实现|修改|构建|调试|重构)/.test(input)) score += 40
    if (/(summary|summarize|wrap up|finalize|conclude|总结|收尾)/.test(input)) score += 15
  }

  if (toolId === 'codex') {
    if (/(review|audit|lint|test|coverage|verify|检查|审查|测试|验证)/.test(input)) score += 45
    if (/(diff|patch|repo|repository|regression|补丁|仓库|回归)/.test(input)) score += 20
  }

  if (toolId === 'opencode') {
    if (/(brainstorm|explore|variant|creative|prototype|concept|头脑风暴|探索|创意|原型)/.test(input)) score += 40
    if (/(ux|ui|copy|content|design|设计|文案)/.test(input)) score += 20
  }

  return score
}

const TOOL_TIE_BREAKER: Record<VibeToolId, number> = {
  claude: 0,
  codex: 1,
  opencode: 2,
}
...
export function planToolRoute(
  text: string,
  mode: ToolExecutionMode | undefined,
  tools = getVibeTools(),
  ...
  const ranked = executableTools
    .map((tool) => {
      ...
      if (sessionToolSet.has(tool.id)) score += 20
