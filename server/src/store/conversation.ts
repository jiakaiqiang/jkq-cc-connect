...
function uniqueStrings(items: Array<string | null | undefined>, limit = 8) {
...
function toolLabel(tool: VibeToolId | null) {
  switch (tool) {
    case 'claude':
      return 'Claude'
    case 'codex':
...
function extractFileCandidatesFromText(text: string) {
...
function extractTouchedFiles(messages: Message[], state: ConversationState) {
  const candidates: string[] = [...state.touchedFiles]
...
function extractOpenQuestions(messages: Message[], state: ConversationState) {
  const candidates: string[] = [...state.openQuestions]
...
function extractRecentDecisions(messages: Message[], state: ConversationState) {
  const candidates: string[] = [...state.recentDecisions]
...
function buildWorkingMemory(
  ...
  if (!parts.length) return state.workingMemory
  return parts.join('；')
}

function buildCanonicalSummary(messages: Message[], state: ConversationState, touchedFiles: string[]) {
  ...
  if (!parts.length) return state.canonicalSummary
  return parts.join('；')
}
...
export function refreshConversationStateFromRecentHistory(sessionId: string, database?: Database.Database) {
