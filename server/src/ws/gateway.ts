...
export class WSGateway {
  ...
  private async handleMentionConversation(session: ReturnType<typeof getSession>, text: string, mentions: AgentMention[]) {
    if (!session) return
    ...
    const collaboratorAgents = mentions
      ...
      .filter((agent): agent is ToolAgentInfo => !!agent)
    ...
    for (const collaborator of collaboratorAgents) {
      ...
      const collaboratorResult = await manager.start(
        session.projectDir,
        text,
        tool.id,
        {
  ...
  private resolveMentionAgent(tool: VibeToolInfo, mention: AgentMention) {
    return tool.agents.find(agent => agent.id === mention.agentId || agent.name === mention.name) || null
  }

  private buildLeadAgentPreamble(tool: VibeToolInfo, leadAgent: ToolAgentInfo) {
  ...
  private buildLeadSummaryPreamble(
    ...
    leadAgent: ToolAgentInfo,
    userText: string,
    collaboratorReplies: Array<{ agent: ToolAgentInfo; reply: string }>,
  ) {
